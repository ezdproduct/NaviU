...
import { User, LoginCredentials } from '@/types'; // Import User và LoginCredentials từ shared types
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchUser = useCallback(async () => {
    ...
    const storedUser = getUser(); // Lấy user từ storage, giờ đây là kiểu User
    if (token && storedUser) {
      try {
  ...
  const login = async (credentials: LoginCredentials): Promise<string> => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setUser(loggedInUser); // Lưu trực tiếp đối tượng User đã được chuẩn hóa
        setIsAuthenticated(true);
  ...
  const updateUserInfo = (newUser: User) => {
    ...
    const token = getToken();
    if (token && newUser) {
        saveToken(token, newUser); // Lưu trực tiếp đối tượng newUser
    }
  };
